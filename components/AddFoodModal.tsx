	...
	useEffect(() => {
		if (isOpen) {
			if (editFood) {
				...
				// Set date and time from editFood timestamp
				if (editFood.timestamp) {
					setSelectedDateTime(new Date(editFood.timestamp))
				} else {
					// Fallback to current date/time if no timestamp
	...
	const handleSubmit = (e: React.FormEvent) => {
		...
		if (nonEmptyInputs.length > 0 && caloriesNum > 0) {
			...
			if (isEditMode && editFood && onUpdate) {
				...
				onAdd(formattedName, caloriesNum, proteinNum, timestamp)
			}
		}
	}
			...
			<form onSubmit={handleSubmit}>
				...
				<div className='mb-3'>
					...
					<button
						...
						disabled={isCalculating}
						className='w-full px-4 py-2 text-sm font-medium text-[#1c1c1c] border border-gray-300 rounded-lg hover:bg-gray-50 transition-colors flex items-center gap-2 disabled:opacity-50 disabled:cursor-not-allowed'
					>
						<svg
							xmlns='http://www.w3.org/2000/svg'
							width='18'
